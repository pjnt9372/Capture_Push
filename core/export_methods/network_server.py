...
class DataExportHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            if path == "/":
                ...
                self._handle_schedule_data()
            elif path == "/api/grades":
                self._handle_grades_data()
    ...
    def _handle_schedule_data(self):
        """处理课表数据请求"""
        logger.info("收到课表数据请求")
        try:
            ...
            self._handle_error(f"获取课表数据失败: {str(e)}")
    
    def _handle_grades_data(self):
        """处理成绩数据请求"""
        logger.info("收到成绩数据请求")
        try:
            data = export_grades_only()
            if isinstance(data, dict) and "status" in data and data["status"] == "error":
                self._send_json_response({"error": data["message"]}, 500)
            else:
                response = {
            ...
            self._handle_error(f"获取成绩数据失败: {str(e)}")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "api_endpoints": [
                ...
                f"http://{local_ip}:{self.port}/api/schedule" if local_ip else f"http://localhost:{self.port}/api/schedule",
                f"http://{local_ip}:{self.port}/api/grades" if local_ip else f"http://localhost:{self.port}/api/grades",
